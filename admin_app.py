    ...
    if maps_dir.exists():
        for f in sorted(maps_dir.glob("*.pmtiles")):
            ...
            try:
                ...
                result = subprocess.run(["/usr/local/bin/pmtiles", "show", str(f)], capture_output=True, text=True, timeout=5)
                zoom = next((re.search(r"max zoom:\s*(\d+)", line).group(1) for line in result.stdout.split("\n") if "max zoom" in line), "-")
            except Exception:
                zoom = "-"
            maps.append({
                "filename": f.name,
                "stem": f.stem,
                "name": "Base World Map" if f.name == "base.pmtiles" else f.stem.upper() if len(f.stem) <= 3 else f.stem.replace("-", " ").replace("_", " ").title(),
...
MAPS_DIR_JSON = "/opt/xprep/maps/directory.json"
...
def api_maps_directory_download():
    ...
    dest = str(Path("/opt/xprep/maps/data") / Path(filename).name)
    parts = cmd_str.split()
    try:
    ...
    def do_extract():
        ...
        status_file.write_text(_json.dumps({"state":"downloading","filename":filename,"started":int(time.time())}))
        result = subprocess.run(parts, timeout=7200, capture_output=True, text=True)
        if result.returncode == 0:
            status_file.write_text(_json.dumps({"state":"done","filename":filename}))
        else:
            status_file.write_text(_json.dumps({"state":"error","filename":filename,"error":result.stderr[-300:]}))
    threading.Thread(target=do_extract, daemon=True).start()
